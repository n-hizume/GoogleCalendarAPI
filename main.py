...
def input_idx(list):
  ...
  while True:
    try:
      ...
      print(f'(0~{size-1})の整数値を入力する必要があります. You Must input integer:(0~{size}).')
      continue
...
def main():
  ...
  while True:
    ...
    idx = input_idx(['終日(All day)', '時間指定(Normal)', '時間指定[タイトルに時間を含める](Normal[The title include times info])'])
      ...
      if idx == 0:
        for my_date in data:
          try:
            ...
            continue
          event = AllDayEvent(st_date=st_date, fn_date=fn_date, title=title)
        ...
        for my_datetime in data:
          ...
          if idx == 2:
            ...
            new_title = title
          event = NormalEvent(st_dtime=st_datetime, fn_dtime=fn_datetime, title=new_title)
          manager.write(event_id, event)
          cout_register_event(event)
      

      print(f'まだ {account} の編集を続けますか？ Do you continue?')
      idx = input_idx(['違う月を入力(Other month)', '違う予定を入力(Other event)', '終了(finish)'])
      if idx == 0:
        continue
      if idx == 1:
        break
